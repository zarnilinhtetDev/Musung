...
class UserApiController extends Controller
{
    ...
    public function postUser(Request $request)
    {
        ...
        $note = $request->note;
        $sql = DB::insert("INSERT INTO users (name,username,password,email,role,line_id,is_delete,remark,created_at) VALUES (?,?,?,?,?,?,?,?,NOW())", [$name, $username, $password, $email, $role, $line_id, 0, $note]);
        DB::disconnect('musung');
        if ($sql == true) {
