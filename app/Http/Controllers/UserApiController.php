...
class UserApiController extends Controller
{
    ...
    {
        $sql = DB::select("SELECT id,name,username,password,email,role,line_id,active_status,is_delete,remark,created_at,updated_at FROM users WHERE role!='99' ORDER BY id ASC");
    ...
    public function postUser(Request $request)
    {
        ...
        $note = $request->note;
        $sql = DB::insert("INSERT INTO users (name,username,password,email,role,line_id,active_status,is_delete,remark,created_at) VALUES (?,?,?,?,?,?,?,?,?,NOW())", [$name, $username, $password, $email, $role, $line_id, 1, 0, $note]);
        DB::disconnect('musung');
        if ($sql == true) {
